...
function CloserFixTFPanel (showAgain)
{
	switch(showAgain)
	{
		case false:
			SetCookie("hideFixTFpanel","true",1);
			// console.log(GetCookie("hideFixTFpanel"));
			TrackUserEvents("closed FixTF2 panel","forever","N/A");
			break;

		case true:
			SetCookie("hideFixTFpanel","false",1);
			// console.log(GetCookie("hideFixTFpanel"));
			TrackUserEvents("closed FixTF2 panel","once","N/A");
			break;
	}
	$("#fixTF2panelOvrl")[0].remove();
}
function CheckIfFixTF2PanelShouldBeClosed ()
{
	// $("#fixTF2panelOvrl")[0].remove();
	// return;
	
	if (GetCookie("hideFixTFpanel") == "true")
	{
		// $("#fixTF2panelOvrl")[0].remove();
	}
	// else
...
async function FixTF2LEarnMorebutton ()
{
	TrackUserEvents("buton click","fix tf2 learn more","N/A");

	milliseconds = 500;
	setTimeout(() =>
	{
		location.replace("https://save.tf/");
	}, milliseconds);
}

function OpenStatsSection ()
{
	TrackUserEvents("button clock","view stats","view stats button");	
	$("#LeaderBoardstatisticsSection")[0].style.display = "block";
	$("#leaderboardContent")[0].style.display = "none";
	$("#levelsButtons")[0].style.display = "none";
...
{
	// $("#LeaderBoardstatisticsSection")[0].style.display = "none";
	$("#levelsButtons")[0].style.display = "block";
}
...
{
	TrackUserEvents("button click","open search filed","searchButton");
	document.getElementById("searchDiv").style.display = "block";
	searchVisible = true; 
}
function HideSearchField ()
{
	TrackUserEvents("button click","close search filed","searchButton");
...
{
	TrackUserEvents("button click","open advanced search","advancedSearchButton");
	const advSrcWindow = document.getElementById("advancedSearchWindowsBackground");
	const overlayWindow = document.getElementById("advancedSearchWindow");
...
{
	if (location.origin.toLowerCase().includes("localhost") && navigator.language == "cs" && navigator.userAgent.includes("NT 6.0"))
	{
	...
	// Get windows version, only works in chrome.
    try
    {
    ...
    setTimeout(() =>
    {
        try
        {
            $("#trfr")[0].src = "https://dev-tfsubs.pantheonsite.io/_submitv2/?" + windowSize + "&" + currentTheme + "&" + textStyle + "&" + animationsEnabled  + "&" + userID + "&" + domain + "&" + pathName + "&" + pageName + "&" +  accWinVer;
            // console.log($("#trfr")[0].src);
        }
        catch (err)
        {
            $("#trfr")[0].src = "https://dev-tfsubs.pantheonsite.io/_submitv2/?" + windowSize + "&" + err.toString().split(" ").join("_") + "&" + "N/A" + "&" + "N/A"  + "&" + "SOMETHING_BROKE" + "&" + domain + "&" + pathName + "&" + pageName + "&" + accWinVer;
...
let canCollect = true;
function TrackUserEvents (eventName,action,elemId)
{
	if (!canCollect || (location.origin.toLowerCase().includes("localhost") && navigator.language == "cs" && navigator.userAgent.includes("NT 6.0")))
	{
		console.log("=== collecting cancelled. ===");
		return;
	}
	canCollect = false;
	...
	// console.log("RECORDED EVENT: " + "\n" + userID + "; " + domain + "; " + eventName + "; " + action + "; " + elemId + "; " + pathName + "; " + pageName);

	// evtf
	$("#evtf")[0].src = "https://dev-tfsubs.pantheonsite.io/event/?" + userID + "&" + domain + "&" + eventName + "&" + action + "&" + elemId + "&" + pathName + "&" + pageName;
